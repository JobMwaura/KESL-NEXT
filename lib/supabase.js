...
// This file will be configured once you set up your Supabase project
...
export async function insertTerm(termData) {
  try {
    // Calculate confidence level based on examples
    const confidence = calculateConfidence(termData.examples);

    // Prepare term for insertion
    const insertData = {
...
export async function checkVariantMatch(term) {
  try {
    ...
    if (matches.length > 0) {
      console.log('⚠️ Found variant matches:', matches.length);
    }
 ...
 * @param {string} data.harm_type - Type of harm (normalizes_contempt, primes_exclusion, cues_violence, harasses, other)
 ...
 * @param {string} data.related_term_id - ID of related term (KEL-XXXX format)
 * @param {string} data.relation_type - Type of relation (variant_of, similar_to, evolved_from, evolved_to, related)
 * @returns {Promise<Object>} Submitted contribution
 */
