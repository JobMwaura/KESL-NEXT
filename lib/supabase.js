...
export async function fetchTermContributions(termId, type = null) {
  try {
    ...
    const contributions = payload.contributions || payload.data || [];

    // Normalize shape for UI components
    return contributions.map((item) => {
      const typeValue = item.type || item.contribution_type;
      const data = item.data || {};

      let content =
        item.content ||
        data.context ||
        data.harm_description ||
        data.quote ||
        data.relation_type ||
        item.details ||
        '';
      ...
      return {
        ...
        type: typeValue,
        content,
        platform: item.platform || data.platform,
        date_observed: item.date_observed || data.date_observed,
        source: item.source || data.source_url
      };
    });
  } catch (error) {
